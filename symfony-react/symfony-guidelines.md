...
Testing an admin action: the `admin` firewall is separate, so use a form login `testadmin`/`testpass` (`config/packages/test/security.yaml`); `loginUser()` doesn't work reliably with an in-memory provider. With an entity provider (`App\Entity\Admin`), build the admin and pass the firewall name: `$client->loginUser($admin, 'admin')`. A `User` logged into `main` gets redirected to `/admin/login`.

#### Custom CRUD actions need `#[AdminRoute]`

Since EasyAdmin 5.0.13, an action declared with `Action::new(...)->linkToCrudAction('foo')`
requires the attribute on the controller method, or route generation ignores it and the
page 500s at render:
...
The bundle's own `UPGRADE.md` states it. Check with `bin/console debug:router | grep export`.

#### A page extending the EA layout only works through `ea_url()`

Any template that extends `@EasyAdmin/layout.html.twig`, `@EasyAdmin/page/content.html.twig`
included, returns a 500 on its bare URL: `Impossible to access an attribute ("i18n") on a
null variable`. The layout does `{% set ea = ea() %}` then reads `ea.i18n` with no guard,
and the context is only set on URLs generated by `ea_url()` / `AdminUrlGenerator`, which
carry `?routeName=…`. From the menu it works, from a bookmark or a mailed link it breaks.
Still the case in 5.5.1.

Three consequences worth knowing before choosing:

- **Bounce onto the contextual URL** at the top of the action rather than rendering:

  ```php
  if (null === $request->attributes->get(EA::CONTEXT_REQUEST_ATTRIBUTE)) {
      return $this->redirect(
          $adminUrlGenerator->setRoute('<route>', $request->query->all())->generateUrl()
      );
  }
  ```

  Pass `$request->query->all()` so the parameters survive the bounce.
- **Internal links use `ea_url()->setRoute(...)`, never `path()`.** EA carries the
  parameters in `routeParams`, not flat, so read them on all three channels:
  `query->getInt()`, `query->all('routeParams')`, `attributes->getInt()`.
- **The rendering cannot be tested.** The test client builds no context, so the page 500s
  even with the right role. Assert the redirect, never the HTML. A `redirectToRoute()`
  towards such a page has the same problem, which is why its functional test passes while
  production breaks.

**A standalone page avoids all of it**: a full HTML document, no `{% extends '@EasyAdmin' %}`.
The `^/admin` firewall still gates it and a test can assert a 200. Prefer it for a rich
custom page, and keep the EA layout for what genuinely belongs inside the back office.

#### A React back-office page with its own API

Expose its routes under `/admin/api/...`, not `/api/...`, so they stay inside the `admin`
firewall and its session, with an explicit `#[IsGranted]` anyway. Nelmio only scans
...
**Exception: a response you intend to stream must not come from a retrying client.**
`RetryableHttpClient` returns an `AsyncResponse`, and `stream()` only accepts a response
its own client produced. A controller streaming an upstream SSE response through
`$this->httpClient->stream($upstream)` therefore dies on
`Argument #2 must be Traversable|array, AsyncResponse given`, a 500 on the endpoint. A
service that both streams and does ordinary calls carries two clients: the scoped one
with the retry for everything else, the plain `@http_client` for the streamed call, with
an absolute URL since it has no `base_uri`. The trap is that hardening the security
config is what introduces it, and no test catches it unless a functional test covers the
streamed route.
...
`ORDER BY` (`OrderByWalker.php`, the hint set by `sort`/`direction` or
`defaultSortFieldName`). Any boost ordering placed in the QueryBuilder, a
`isFeaturedFirst DESC` say, therefore ends up **behind** the user's sort and stops having
any visible effect.

When the ordering matters, give the repository the whole `ORDER BY`: the boost, the
requested field validated against an internal allow-list, and a tie-break on the id. The
controller then strips `sort` and `direction` from the query parameters so Knp adds
nothing, and `defaultSortFieldName` / `sortFieldAllowList` come off the `paginate()` call.
Stripping them also neutralises legacy sort parameters from old bookmarks, which
otherwise 500 on `InvalidValueException`.

Plain table pagination with no boost keeps the Knp sort, which is what it is good at.
...
### Icons: `symfony/ux-icons`, never an inline `<svg>`

```twig
{{ ux_icon('lucide:arrow-left', {class: 'h-4 w-4'}) }}
...
Two things behave differently from an ordinary response.

**Reading the body.** The test client already runs the callback during
`$client->request()`, so a later `sendContent()` is a no-op and an `ob_start()` around it
captures nothing. `getContent()` on the `StreamedResponse` returns `false`. What was
actually emitted is in the BrowserKit response: read
`$client->getInternalResponse()->getContent()`. No output buffering to manage.

**Mocking the upstream.** `stream()` has to be called on the client that produced the
response, or it throws "Response not created by this client". So wire the **same shared
mock instance** in `config/services_test.yaml`, both on the upstream service's streaming
client and on the controller's own client. The mock returns a `MockResponse` whose body is
a `\Generator<string>` of `data: {...}\n\n` chunks for the controller to reparse.

Worth the effort: a streamed endpoint is exactly where a 500 hides, since the E2E journey
...
## 19. Deploying on CleverCloud

**A push deploys.** The GitHub integration maps a branch to an application: `main` goes to
production, `preprod` to preproduction. There is no separate deploy step, so pushing `main`
is a production release, immediately. To validate on preproduction first, push `preprod`
and only merge `main` afterwards.

Each project's `AGENTS.md` carries its own aliases and application IDs. Follow a deploy
with `clever activity --app <ID>` (the `--alias` form does not resolve from the working
directory when the app is not linked) and read a build with
`clever logs --alias <alias> --since <ISO> --until <ISO>`. Always bound `--until`: without
it the command streams and hangs.

### `COMPOSER_AUTH` is not optional

The builder runs `composer install` with source fallback disabled, so an unauthenticated
GitHub dist download failing (504 on `api.github.com`, 429 on `codeload`) fails the whole
build in seconds. Set
`COMPOSER_AUTH={"github-oauth":{"github.com":"<fine-grained PAT>"}}` on every application.
Read access to public repositories is enough. When the token expires, deploys start
failing the same way again, so it is worth suspecting early.

Production stays on the previous version while a deploy fails, so a failed build is not an
outage. `clever restart --alias <alias>` redeploys the same commit, which is the right
reflex for a transient failure. Two signatures worth recognising: cascading 429s from
GitHub (check its status page before reading your own diff), and build logs stopping dead
mid-`pnpm install` with no error line, which is the builder being killed, not your code.

### `post_build.sh`, and where a data back-fill goes

The build hook regenerates the SDK, builds the front and applies the schema. A data
back-fill is a one-shot `app:*` console command, wired into the hook **before** the
schema-update line when the data has to be fixed while the columns still have their old
...
Schema update converts a legacy `SERIAL` column to `GENERATED BY DEFAULT AS IDENTITY`, and
Postgres creates a **fresh** sequence, suffixed `_id_seq1`, starting at 1, on a table that
already holds higher ids. The next insert then collides with an existing row, surfacing in
production as a `UniqueConstraintViolation` on the primary key. A one-shot `setval` fixes
the day, an `app:sync-identity-sequences` command wired into the hook keeps it fixed
across later schema updates.
