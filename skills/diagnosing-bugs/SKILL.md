...
- for a Sentry bug: start from the real event (payload, stack, breadcrumbs through the
  ...
  that plumbing: lean on it here and to resolve the issue at step 5.
...
No fixing to see what happens: every speculative fix pollutes the area and destroys the
...
## 6. Write `.claude/plan.md`

`/review-diff` runs in a forked context, so this file is the only intent it gets. A fix
that leaves it empty gets reviewed against the guidelines alone.

- What broke, and the root cause in one line.
- The blast radius of the fix.
- The regression test, under "Tests owed", ticked once it is green.
