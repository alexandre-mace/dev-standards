...
The fork is decided **when the project starts**, by one question: is the content the
product, or is the interface the product?

- **The content is the product.** Pages have to be indexed, shared with a preview, and
  readable by a stranger on first paint. That is `next/`: static-first, data baked at
  build, SEO and OpenGraph earn their keep.
- **The interface is the product.** The user arrives to *do* something, the screens are
  generated by what they type and click, and no crawler needs any of it. That is here,
  where the value sits in typed routing, typed data and speed of iteration.

A login is the most common sign of the second case, not its definition: an internal
tool that happens to sit on an open URL belongs here just as much.

The case that decides itself the other way is worth naming, because it is the whole
`climat/` family: **a public simulator whose reach is the point stays on `next/`**. Its
interface is the product, yes, but being found, shared and previewed matters more than
anything this stack would buy. A static landing carries the SEO and the tool runs in the
browser, which is precisely what Next does well. Ask what would hurt more: a slower
build loop, or nobody finding the tool.
