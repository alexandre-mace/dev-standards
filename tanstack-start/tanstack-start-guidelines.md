# TanStack Start Guidelines: apps behind auth

> Conventions for TypeScript apps that live entirely behind a login: dashboards,
> internal tools, products. Pragmatic, not dogmatic.
...
The fork is decided **when the project starts**, and it is decided by one question:
does a stranger, or a crawler, ever see a page?

- **A public site**, where content must be indexed and shareable, goes to `next/`.
  Static-first, data baked at build, SEO and OpenGraph matter.
- **An app entirely behind a login** comes here. Nobody indexes it, server rendering
  buys almost nothing, and the value sits in typed routing, typed data and speed
  of iteration.
...
- Reaching for this stack for a public content site. That is `next/`.
...
- Copying a Next.js idiom by reflex: there are no React Server Components here, and
  `use client` means nothing.
